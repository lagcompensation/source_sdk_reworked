...
namespace csgo
{
	...
	void render::create_font( void** font, const std::string& family, int size, bool bold ) {
		...
		D3DXCreateFontA( csgo::i::dev, size, 0, bold ? FW_BOLD : FW_LIGHT, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, 0, DEFAULT_PITCH | FF_DONTCARE, family.data( ), &d3d_font );
		*font = d3d_font;
	}

	void render::create_font( void** font, const std::string& family, int size, int width ) {
		ID3DXFont* d3d_font = nullptr;
		D3DXCreateFontA( csgo::i::dev, size, 0, width, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, 0, DEFAULT_PITCH | FF_DONTCARE, family.data( ), &d3d_font );
		*font = d3d_font;
	}
